...
7. **Arrêt brutal du PC → rollback Mongo** (perte users/clients d'intégration alors que Postgres garde tout). Toujours `wsl --shutdown` avant d'éteindre. Réparation : tâche `opencrvs-seed` (+ recréer le client d'intégration). ⚠️ Le re-seed **régénère aussi les UUIDs des lieux** → mettre à jour `OPENCRVS_LOCATION_ID` dans `.env` (sinon les notifications partent vers un bureau fantôme et la file reste vide). Retrouver le bon UUID : `docker exec opencrvs-mongo1-1 mongo user-mgnt --quiet --eval "print(db.users.findOne({username:'k.mweene'}).primaryOfficeId)"`.
