...
Évaluation sur **17 documents typés/imprimés** (un par pays, hors manuscrits — voir plus bas), générée automatiquement à partir de `runs/<doc>/report.json` par `tools/evaluate_batch.py`.
...
| Indicateur | Valeur |
|---|---|
| Documents évalués | 17 |
| Champs détectés en moyenne / document | 23.6 |
| **% de champs auto-acceptés en moyenne** (confiance suffisante, aucune relecture requise) | **57.3 %** |
| **Champs OpenCRVS pré-remplis en moyenne / document** | **7.2** |
| Répartition des scores de confiance (tous champs, tous documents) | haute 235 (58.5%) · moyenne 78 (19.4%) · basse 89 (22.1%) |
...
| **Rappel** | **84.5 %** |
| **F1** | **91.6 %** |
| Accuracy globale | 86.1 % |
| Champs évalués (avec double-passe page/crop) | 310 sur 402 champs détectés |
...
- **Précision** (235 / 235) : parmi les champs que le système présente comme fiables, combien le sont réellement — l'indicateur qui compte le plus pour OpenCRVS, puisqu'il mesure le risque de préremplir une valeur fausse avec assurance.
- **Rappel** (235 / 278) : parmi les champs réellement bons, combien le système a osé pré-remplir plutôt que renvoyer à la relecture par prudence.
...
Détail par document (pays, % auto-accepté, méthode de localisation, champs OpenCRVS pré-remplis, matrice de confusion…) : `metriques_par_document.csv` dans ce même dossier — 21 lignes, y compris les 4 documents manuscrits marqués `handwritten=True` et exclus des chiffres ci-dessus.
