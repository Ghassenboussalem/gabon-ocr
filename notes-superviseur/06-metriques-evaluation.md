...
Évaluation sur **21 documents typés/imprimés** (un par pays, hors manuscrits — voir plus bas), générée automatiquement à partir de `runs/<doc>/report.json` par `tools/evaluate_batch.py`.

⚠️ **1 document(s) non traités** — quota gratuit journalier des 3 clés Gemini épuisé (429 Too Many Requests) pendant ce lot : sc_naissance_1987. Ce n'est pas un échec du pipeline — c'est la limite connue du free tier (~250 requêtes/jour/clé), déjà documentée comme raison de bascule vers un VLM local en production. Nouvel essai possible le lendemain (quota réinitialisé) ou avec des clés supplémentaires.

**5 document(s) manuscrits exclus des métriques** (cd_acte_2023, cm_acte_1977, gabon_p4, rw_acte_2013, sn_extrait_1997) — la reconnaissance d'écriture manuscrite est un problème distinct, nettement plus dur, que la lecture d'actes tapés/imprimés ; les mélanger tirait les chiffres vers le bas et ne reflétait pas la performance réelle sur la cible principale du pipeline. Ils restent traités normalement par le pipeline et dans `samples/` — seulement retirés de ce calcul.
...
| Champs détectés en moyenne / document | 22.9 |
| **% de champs auto-acceptés en moyenne** (confiance suffisante, aucune relecture requise) | **56.5 %** |
| **Champs OpenCRVS pré-remplis en moyenne / document** | **7.0** |
| Répartition des scores de confiance (tous champs, tous documents) | haute 279 (58.0%) · moyenne 93 (19.3%) · basse 109 (22.7%) |
...
| **Rappel** | **85.1 %** |
| **F1** | **91.9 %** |
| Accuracy globale | 86.7 % |
| Champs évalués (avec double-passe page/crop) | 369 sur 481 champs détectés |
...
- **Précision** (279 / 279) : parmi les champs que le système présente comme fiables, combien le sont réellement — l'indicateur qui compte le plus pour OpenCRVS, puisqu'il mesure le risque de préremplir une valeur fausse avec assurance.
- **Rappel** (279 / 328) : parmi les champs réellement bons, combien le système a osé pré-remplir plutôt que renvoyer à la relecture par prudence.
...
**2 document(s) sont tombés sur le pack générique « zz » au lieu d'un pack pays dédié.** Le pack générique n'ayant pas d'ancres spécifiques, la localisation interpole davantage et plafonne la confiance de chaque champ à la bande « moyenne » au mieux, d'où un taux d'auto-acceptation à 0 % — cohérent avec le gate d'honnêteté (mieux vaut sous-noter que sur-noter), mais cela vaut la peine de vérifier pourquoi la détection automatique du pays n'a pas choisi le pack dédié existant sur ces documents (détail dans le CSV).

Détail par document (pays, % auto-accepté, méthode de localisation, champs OpenCRVS pré-remplis, matrice de confusion…) : `metriques_par_document.csv` dans ce même dossier — 26 lignes, y compris les 5 documents manuscrits marqués `handwritten=True` et exclus des chiffres ci-dessus.
